...
public class GoApi {
	...
	private void initDeviceInfo(){
		deviceInfo.setDeviceId("1234abcd");
	...
	public void ptcLogin(String username, String password) {
		OkHttpClient http = new OkHttpClient();
		try {
			PtcCredentialProvider provider = new PtcCredentialProvider(http, username, password);
			go = new PokemonGo(provider, http);
			go.setDeviceInfo(deviceInfo);
		} catch (LoginFailedException | RemoteServerException | AsyncPokemonGoException e) {
			e.printStackTrace();
	...
	public void catchPokemon(){
		...
		try {
			...
			for (CatchablePokemon catchablePokemon : catchablePokemons) {
				...
				int cp = encounterResult.getPokemonData().getCp();
				CatchResult result = catchablePokemon.catchPokemon();
				if (listener != null) {
					if (result.isFailed())
						listener.onFlee(chat, catchablePokemon, cp);
					else
						listener.onCaought(chat, catchablePokemon, cp);
				}
				try {
		...
		try {
			...
			for (Pokestop pokestop : pokestops) {
				if (pokestop.canLoot()) {
					pokestop.loot();
					if (listener != null) {
						listener.onPokeStop(chat, pokestop);
					}
					try {
						Thread.sleep(5 * 1000);
					} catch (InterruptedException e) {
